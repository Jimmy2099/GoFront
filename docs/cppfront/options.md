...
Go2 supports C++-style compile-time overloading for top-level functions and `type ... class` member functions. Declare the same Go2 `func` name more than once with different parameter types or parameter counts, such as `func Print(value int)`, `func Print(value float64)`, and `func Print(value string)`. The normalizer preserves the name and cppfront resolves the generated C++ overload set. Return type alone does not make a distinct overload.

Go2 class operators use the same declaration form: `func operator+(other Box) Box { ... }` lowers to a public native C++ `operator+`, so `first + second` uses that overload. The parameter and return types remain Go2 types; no C++ declaration syntax is required in the Go2 source. The same form supports C++'s overloadable arithmetic, comparison, logical, unary, increment/decrement, bitwise, assignment, call (`operator()`), subscript (`operator[]`), comma, arrow, and allocation/deallocation operator names. Go2 explicitly rejects `.`, `.*`, `->*`, `::`, `sizeof`, `?:`, and preprocessor `#`, matching its operator policy. As in C++, overloaded `&&`, `||`, and `,` do not retain built-in short-circuit or sequencing behavior.
